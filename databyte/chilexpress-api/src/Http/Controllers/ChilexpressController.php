...
namespace Databyte\ChilexpressApi\Http\Controllers;

use Databyte\ChilexpressApi\Http\Requests\QuoteTest\QuoteRequest;
...
class ChilexpressController extends Controller
{
    ...
    {
        $counties = ChilexpressCounty::all()->pluck('countyName', 'countyCode')->toArray();
        $product_types = [];
        $delivery_services = [];
        ...
        return view('chilexpress-api::test_form', compact('counties', 'product_types', 'delivery_services'));
    }
    ...
    public function quote(QuoteRequest $request)
    {
        try {
            $client = new Client();
            $res = $client->request('POST', chilexpressapisetting('chilexpress-quote-url'), [
                'headers' => [
                    'Content-Type' => 'application/json',
                    'Cache-Control' => 'no-cache',
                    'Ocp-Apim-Subscription-Key' => chilexpressapisetting('ocp-apim-subscription-key'),
                ],
                'json' => [
